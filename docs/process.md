...
1. 骨格テンソル保存時に ``pickle_protocol=4`` を指定し、I/O を最適化。
2. Edge IoU のしきい値を各バッチの最大値から算出する方式を整理。
3. 学習・検証分割に重複が無いか ``assert`` で検査する安全策を追加。
4. 入力ノイズの減衰を線形スケジュールとし、``Mean_Edge_Width`` は 25 % のバッチのみ計測。
